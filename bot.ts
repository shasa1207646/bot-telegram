...
dotenv.config();
// Кэш участников — обновляется не чаще раз в 5 минут
...
async function playSound(
  ...
  console.log(`📁 файл существует: ${fs.existsSync(soundPath)}`);
  ...
  // Обработка зависания соединения: signalling/connecting → принудительный реконнект
    ...
    if (
      ...
      try {
        ...
        if (connection.state.status !== VoiceConnectionStatus.Destroyed) {
          console.warn('⚠️ Соединение зависло — уничтожаем');
          tg.sendMessage(chatId, '❌ Не удалось подключиться к голосовому каналу (таймаут). Попробуй ещё раз.');
          connection.destroy();
        }
      }
    }
  ...
  connection.once(VoiceConnectionStatus.Ready, () => {
    console.log('✅ Connection Ready — начинаем воспроизведение');
    startPlaying();
  });

  connection.on(VoiceConnectionStatus.Disconnected, () => {
    console.log('🔌 Connection Disconnected — destroy');
